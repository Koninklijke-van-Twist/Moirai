 ...
 * Mímir beheert environments — Moirai heeft alleen de API-key (+ optioneel $mimirBase) nodig.
 *
 ...
 *
 * max_age-beleid: nightly.php bestaat (aging-alerts) maar fetcht geen OData; geen hourly.php.
 * UI/on-demand houdt bestaande TTLs (default odata_get_all = 300).
 * MOIRAI_NIGHTLY_MAX_AGE (14400) is gereserveerd voor eventuele future OData-nightly paths.
 */

/** Mímir max_age for nightly-style OData fetches (4h). Current nightly.php does not fetch OData. */
const MOIRAI_NIGHTLY_MAX_AGE = 14400;
...
function odata_mimir_request(string $method, string $path, ?array $jsonBody = null): array
{
    ...
    $opts = [
        ...
        CURLOPT_USERAGENT => 'Moirai-MimirClient/1.0',
    ];
    if ($jsonBody !== null) {
 ...
 * $odataQuery gebruikt app-keys zoals $select / $filter.
 *
...
if (odata_mimir_enabled()) {
    // Moirai may set $environment as string or list; only fill when completely unset/empty string.
    if (!isset($environment)) {
        $environment = 'mimir';
    } elseif (is_string($environment) && trim($environment) === '') {
        $environment = 'mimir';
    }
...
function odata_get_all(string $url, array $auth, $ttlSeconds = 300): array
{
    ...
    if (odata_mimir_api_key() !== '') {
        // Mímir beheert de BC-cache (max_age); Moirai-filecache / live-paginering worden overgeslagen.
